...
547a9cf61e0b40f4cd41b1552aacb328
...
url = "http://ws.audioscrobbler.com/2.0/?method=artist.search&artist=Lil&limit=50&api_key=547a9cf61e0b40f4cd41b1552aacb328&format=json"
...
for artist in artists:
    total_listeners = total_listeners + int((artist['listeners']))
...
for artist in artists:
    if (artist['name']) == "Lil Jon":
        print("Lil Jon's mbid is", artist['mbid'])
...
url = "http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&mbid=a95384b1-6aec-468c-ae0d-8c6daf87c4c2&api_key=547a9cf61e0b40f4cd41b1552aacb328&format=json"

response = requests.get(url)

data = response.json()

print(data['artist']['bio'])
...
for artist in artists:
    if artist['mbid'] != "":
        mbids = artist['mbid']
        print(f'http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&mbid={mbids}&api_key=547a9cf61e0b40f4cd41b1552aacb328&format=json')
...
for artist in artists:
    if artist['mbid'] != "":
        mbids = artist['mbid']
        mbid_url = (f'http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&mbid={mbids}&api_key=547a9cf61e0b40f4cd41b1552aacb328&format=json')
        response = requests.get(mbid_url)
        data = response.json()
        names.append(data['artist']['name'])
print(names[:3])
...
base_url = "http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&api_key=547a9cf61e0b40f4cd41b1552aacb328&format=json&mbid="

for url_mbid in mbid[:3]:
    mbid_url = base_url+url_mbid
    response = requests.get(mbid_url)
    data = response.json()
...
for url_mbid in mbid[:3]:
    ...
    for mbid_tag in mbid_tags:
        mbid_tags = data['artist']['tags']['tag']
        print(name, mbid_tag['name'])
...
for url_mbid in mbid[:10]:
    ...
    for mbid_tag in mbid_tags:
        mbid_tags = data['artist']['tags']['tag']
    if mbid_tag['name'] == "rap" or mbid_tag['name'] == "hip hop" or mbid_tag['name'] == "swag" or mbid_tag['name'] == "crunk" or mbid_tag['name'] == "dirty south":
        print(name, "is a rapper.")
    else:
        print(name, "is not a rapper.")
...
print(count/10*100, "percent 'lil' results are rappers.")
